...
function guardarLocalS(name, x, f){
    localStorage.setItem('persona', JSON.stringify( name));
    localStorage.setItem('cantidad', x)
    localStorage.setItem('precioF', f)
}
...
let x = 0;
let precioFinal = 0;
function guardarCarrito(name, precio) {
    ...
    let valorT = parseInt(precio) * parseInt(nCantidad);
    
    x += parseInt(nCantidad);
    precioFinal += parseInt(valorT);
    let pedido2 = {nombre: name, cant: nCantidad}
        pedidoCarrito.push(pedido2)
    //     // console.log(x)
    ...
    alert('Agregado')
    guardarLocalS(pedidoCarrito, x, precioFinal)
   pintarCartas()
}